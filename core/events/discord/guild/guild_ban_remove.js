...
module.exports = async ({ client, ban }) => {
    ...
    if (!guild.logger_member_ban_remove || !guild.conf_logger) return

    // Permissão para ver o registro de auditoria, desabilitando o logger
    if (!await client.permissions(ban, client.id(), PermissionsBitField.Flags.ViewAuditLog)) {
        await updateGuild(client, guild.id, { logger_member_ban_remove: false })
    ...
    let razao = "", network_descricao = "", canal_aviso = guild.logger_channel

    // Alterando o canal alvo conforme o filtro de eventos do death note
    if (guild.death_note && guild.death_note_member_ban_remove && guild.death_note_channel)
