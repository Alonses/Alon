...
module.exports = async ({ client, user, interaction, pagina_guia }) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setFields(
            {
                name: `${client.execute("functions", "emoji_button.emoji_button", guild.warn_hierarchy_status)} **${client.tls.phrase(user, "menu.botoes.usar_hierarquia")}**`,
                value: `${client.execute("functions", "emoji_button.emoji_button", guild.warn_hierarchy_timed)} **${client.tls.phrase(user, "mode.warn.com_validade")}**\n${client.emoji(47)} **${client.tls.phrase(user, "menu.botoes.anotacoes")}: \`${guild.warn_hierarchy_strikes}\`**\n${client.emoji(47)} **${client.tls.phrase(user, "mode.warn.advertencias")}: \`${advertencias.length} / 5\`**`,
                inline: true
            },
            {
                name: `${client.defaultEmoji("telephone")} **${client.tls.phrase(user, "menu.botoes.painel_de_controle")}**`,
                value: `${guild.warn_hierarchy_channel ? `${client.emoji("icon_id")} \`${guild.warn_hierarchy_channel}\`\n( <#${guild.warn_hierarchy_channel}> )` : `\`❌ ${client.tls.phrase(user, "mode.network.sem_canal")}\``}`,
                inline: true
            },
            {
                name: `${client.defaultEmoji("time")} **${client.tls.phrase(user, "mode.warn.validade")}**`,
                value: `:wastebasket: **${client.tls.phrase(user, "mode.warn.expira_em")} \`${client.tls.phrase(user, `menu.times.${defaultEraser[guild.warn_hierarchy_reset]}`)}\`**${guild.warn_hierarchy_timed ? "" : `\n( **⛔ ${client.tls.phrase(user, "mode.warn.no_momento")}** )`}`,
                inline: true
            },
            {
    ...
    botoes = botoes.concat([
        { id: "guild_hierarchy_warns_button", name: client.tls.phrase(user, "menu.botoes.usar_hierarquia"), type: client.execute("functions", "emoji_button.type_button", guild.warn_hierarchy_status), emoji: client.execute("functions", "emoji_button.emoji_button", guild.warn_hierarchy_status), data: "1", disabled: !guild.warn_hierarchy_channel },
        { id: "guild_hierarchy_warns_button", name: client.tls.phrase(user, "mode.warn.com_validade"), type: client.execute("functions", "emoji_button.type_button", guild.warn_hierarchy_timed), emoji: client.execute("functions", "emoji_button.emoji_button", guild.warn_hierarchy_timed), data: "2" },
        { id: "guild_hierarchy_warns_button", name: client.tls.phrase(user, "menu.botoes.avisos_previos"), type: 1, emoji: default_emoji["numbers"][guild.warn_hierarchy_strikes], data: "4" }
    ])
