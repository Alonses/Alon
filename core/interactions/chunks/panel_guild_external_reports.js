...
module.exports = async ({ client, interaction, pagina_guia }) => {
    ...
    const guild = await client.getGuild(interaction.guild.id, { reports: true })
    ...
    const embed = new EmbedBuilder()
        ...
        .setFields(
            {
                name: `${client.execute("functions", "emoji_button.emoji_button", guild?.reports.enabled)} **${client.tls.phrase(user, "mode.report.status")}**`,
    ...
    if (pagina === 0) {
        ...
        botoes = botoes.concat([
            { id: "guild_reports_button", name: client.tls.phrase(user, "manu.painel.reports_externos"), type: client.execute("functions", "emoji_button.type_button", guild?.reports.enabled), emoji: client.execute("functions", "emoji_button.emoji_button", guild?.reports.enabled), data: "1" },
